...
function ShowsAdmin({ token, toast }) {
  ...
  const [form, setForm]     = useState({ name: '', showDate: '', showTime: '', imageUrl: '', imagePosition: 'center center' })
  ...
  function resetForm() {
    setForm({ name: '', showDate: '', showTime: '', imageUrl: '', imagePosition: 'center center' })
    setEditing(null)
  }

  function startEdit(show) {
    setEditing(show.id)
    setForm({ name: show.name, showDate: show.showDate || '', showTime: show.showTime || '', imageUrl: show.imageUrl || '', imagePosition: show.imagePosition || 'center center' })
  }
      ...
      <div className="ap-block">
        ...
        <form className="ap-form ap-form--col" onSubmit={handleSave}>
          <div className="ap-form--grid2">
            ...
            <div>
              <label className="ap-label">Fecha del show 📅</label>
              <input
                className="ap-input"
                type="date"
                value={form.showDate}
          ...
          <div className="ap-form-row">
            <div>
              <label className="ap-label">Hora <span style={{ color: 'var(--ap-muted)', fontWeight: 400, fontSize: 11 }}>(opcional)</span></label>
              <input
                className="ap-input"
                type="time"
                value={form.showTime}
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ fontSize: 12, color: 'var(--ap-muted)', lineHeight: 1.5 }}>
                💡 Elegí la fecha y la web ordena los shows sola: el más próximo queda <strong>destacado grande</strong>. Cuando pasa la fecha, el show se mueve solo a <strong>realizados</strong>.
              </div>
            </div>
          </div>
...
function ClubAdmin() {
  // El Club tiene panel propio en /club-admin: pantallas pensadas para operarlo
  // desde el celular, sin el ruido del Prode ni del Mundial.
  return (
    <div className="ap-section">
      <h2 className="ap-section__title">★ Sala Crespo Club</h2>
      <p style={{ color: '#8B9BB4', fontSize: 14, margin: '0 0 22px', maxWidth: 640, lineHeight: 1.6 }}>
        El Club se maneja desde su propio panel: premios de Fortuna Dorada, canjes atados a la carta,
        cuentas de socios y los números del programa.
      </p>
      <a href="/club-admin" style={{
        display: 'inline-flex', alignItems: 'center', gap: 10, textDecoration: 'none',
        padding: '14px 24px', borderRadius: 12, fontSize: 15, fontWeight: 700,
        background: 'linear-gradient(135deg, #C9A84C, #F0D275)', color: '#1a0d05',
        boxShadow: '0 4px 18px rgba(201,168,76,.28)',
      }}>Abrir el panel del Club</a>
    </div>
  )
}
...
function EmailBlastAdmin({ token, toast }) {
  ...
  const TEMPLATES = [
    ...
    { id: 'notif-app', title: '🔔 Activá notificaciones + instalá la app', desc: 'Mail educativo: explica los dos avisos automáticos (30 min antes del partido y los puntos al finalizar) y enseña paso a paso cómo instalar la app y activar las notificaciones en Android e iPhone. Ideal para subir retención.', audienceType: 'base', audience: 'Toda tu base de clientes con email' },
    { id: 'serie', title: '🎰 Torneos 2026 · Camino a la Gran Final', desc: 'Anuncio de la serie: las 4 fechas (3 satélites + Gran Final), $200.000 por satélite, 20 clasificados por torneo y la Gran Final por $2.000.000. Trae las fechas automáticamente de la serie activa. Si hay un satélite con inscripción abierta, el CTA lleva a inscribirse; si no, a ver el camino a la Final. Reutilizable antes de cada satélite.', audienceType: 'base', audience: 'Toda tu base · filtrable por segmento' },
    { id: 'camino-mundial', title: '🏆 Camino Mundial 2026 (página regalo)', desc: 'Anuncia la página interactiva del Mundial: las 48 selecciones, el cuadro de eliminación, planteles con fotos, cada gol con detalle y los números del torneo. Aclara que se actualiza fase por fase hasta el campeón. CTA a www.saladejuegoscrespo.ar/caminomundial2026.html', audienceType: 'base', audience: 'Toda tu base de clientes con email' },
    { id: 'cierre-mundial', title: '🏆 Cierre del Mundial + ganadores del Prode', desc: 'Mail de cierre: España campeón, y el podio del Prode con los 3 ganadores (Katrina, Xavier, Mauro) y sus premios en Free Play. Menciona el bonus Profeta y agradece por participar. CTA a Camino Mundial. Para mandar UNA vez al cerrar el torneo.', audienceType: 'base', audience: 'Toda tu base de clientes con email' },
  ]
